...
variable "application_name" {
  ...
  validation {
    condition = length(var.application_name) <= 26
    error_message = "The variable application_name value is over 26 characters."
  }
}
...
variable "eb_environment_variable_db_host_key" {
  default     = "DB_HOST"
  description = "Database remote host connection environment variable name"
}

variable "eb_environment_variable_db_host_value" {
  description = "Database remote host connection environment variable value"
}
